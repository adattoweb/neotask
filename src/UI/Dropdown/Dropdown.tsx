...
function Button({ children, className, needToRotating = false }:ButtonProps){
    const { isOpen, setIsOpen } = useCheckContext("Dropdown Context", DropdownContext)
    const onClick = () => {
        setIsOpen(prev => !prev)
    }
    return (
        <div className={`${styles.button} ${className} ${isOpen && needToRotating ? styles.rotated : ""}`} onClick={onClick}>{children}</div>
    )
}
...
function Content({ children, className, addTop = 0 }:ContentProps){
    ...
    return createPortal(
        <div ref={currentRef} style={stylesPos} className={`${styles.content} ${className} bg-alpha br-alpha ${isOpen ? "top-in" : "bottom-out"}`} onAnimationEnd={handleAnimationEnd}>
            {children}
        </div>,
        document.body
    )
