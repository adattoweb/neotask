...
export function Button({ onClick, className, children }: ButtonProps) {
   return (
      <div className={`${styles.button} bg-alpha bg-alpha-hover ${className}`} onClick={onClick}>
         {children}
      </div>
   )
}
...
export function Paragraph({ className, fontSize = 14, children }: CustomText) {
   return (
      <p className={`${styles.paragraph} ${className}`} style={{ fontSize: fontSize }}>
         {children}
      </p>
   )
}
