    ...
    return (
        <Dropdown className={`${styles.dropdown} bg-alpha`}>
            ...
            <Dropdown.Content className={clsx(styles.content, "rdp-dropdown__content")}>
                <DayPicker locale={uk} animate required showOutsideDays mode="single" selected={selected} onSelect={setSelected} footer={selected ? `${t("selected")}: ${selected.toLocaleDateString()}` : `${t("pickADay")}.` }/>
            </Dropdown.Content>
        </Dropdown>
    )
}